...
def main(_):
    ...
    # input image
    x_f = tf.compat.v1.placeholder(tf.float32, [None, input_size[1], input_size[1]])
    x_l = tf.compat.v1.placeholder(tf.float32, [None, input_size[0], input_size[1]])
    x_r = tf.compat.v1.placeholder(tf.float32, [None, input_size[0], input_size[1]])


    y_ = tf.compat.v1.placeholder(tf.float32, [None, 2])  # output label
    # note that num_subj is multiplied by 2 because we treat the images that
    # are horizontally flipped as a different subject
    subj_id = tf.compat.v1.placeholder(tf.float32, [None, 2 * num_subj])  # subject index

    # online data augmentation
    f_processed = tf.compat.v1.where(is_aug, pre_process_face_images(x_f, True), pre_process_face_images(x_f, False))
    face = f_processed - mu
    l_processed = tf.compat.v1.where(is_aug, pre_process_eye_images(x_l, True), pre_process_eye_images(x_l, False))
    left_eye = l_processed - mu
    r_processed = tf.compat.v1.where(is_aug, pre_process_eye_images(x_r, True), pre_process_eye_images(x_r, False))
    ...
    est_loss = tf.reduce_sum(input_tensor=(g_hat - y_ * 10.)**2, axis = 1, keepdims=True)


    total_loss = (tf.reduce_mean(input_tensor=est_loss) + 1e-3 * l2_loss +
                  tf.abs(tf.reduce_mean(input_tensor=bias_W_fc[:num_subj,0]))+tf.abs(tf.reduce_mean(input_tensor=bias_W_fc[num_subj:,0]))+
                  tf.abs(tf.reduce_mean(input_tensor=bias_W_fc[:num_subj,1]))+tf.abs(tf.reduce_mean(input_tensor=bias_W_fc[num_subj:,1])))

    cls1_vars = (tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="face") +
                 tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="eye") +
                 tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="combine") +
                 tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="bias"))

    vgg_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="transfer")

    # optimizer
    with tf.compat.v1.variable_scope("LearnRate"):
        global_step = tf.Variable(0, trainable=False)
        learning_rate = tf.compat.v1.train.exponential_decay(1e-3, global_step, 8000, 0.1,
                                                   staircase=True)

    opt1 = tf.compat.v1.train.AdamOptimizer(learning_rate)
    opt2 = tf.compat.v1.train.AdamOptimizer(1e-1 * learning_rate)

    update_ops1 = (tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope="face") +
                   tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope="eye") +
                   tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope="combine"))
