...
def conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME'):
    ...
    return tf.nn.conv2d(input=x, filters=W, strides=strides,padding=padding)
...
def dilated2d(x, W, rate, strides=[1, 1, 1, 1], padding='VALID'):
    ...
    return tf.nn.convolution(input=x, filters=W, strides=strides, padding=padding,
...
def bias_variable(shape, std=0.1, trainable=True):
    ...
    '''
    initial = tf.random.normal(shape, mean=0.0, stddev=std)
